...
int paging_set(uint32_t* directory, void* virt, uint32_t val)
{
    ...
    uint32_t* table = (uint32_t*) (entry & 0xFFFFF000); // Extract only the 20 bits representing the 4096 aligned physical address
    table[table_index] = val;

    return 0;
}

uint32_t paging_get(uint32_t* directory, void* virtual)
{
    uint32_t directory_index = 0;
    uint32_t table_index = 0;
